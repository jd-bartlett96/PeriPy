...
bc_values = np.array([0, None, 0.02], dtype=np.float64)
bc_types = np.array([0, None, 1], dtype=np.float64)

#print(type(r[0][0]), type(r0[0][0]), type(n_list[0][0]), type(n_neigh[0]))

def main():
    K = assemble_K_global(r, r0, n_list, n_neigh, volume, bond_stiffness,
                        bc_values, bc_types)
    print(K)
