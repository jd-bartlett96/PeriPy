...
def update_displacement(
    ...
    u[:, 0] = np.where(bc_types[:, 0] == 0, u[:, 0] + dt * force[:, 0], displacement_bc_magnitude * bc_values[:, 0])
    u[:, 1] = np.where(bc_types[:, 1] == 0, u[:, 1] + dt * force[:, 1], displacement_bc_magnitude * bc_values[:, 1])
    u[:, 2] = np.where(bc_types[:, 2] == 0, u[:, 2] + dt * force[:, 2], displacement_bc_magnitude * bc_values[:, 2])
    return u
