...
@njit(parallel=True)
def update_displacement(nnodes, degrees_freedom,
                        force, u, ud, udd, dt, damping, densities,
                        bc_types, displacement_bc_magnitude, bc_values):
    """
    ...
    for node_id_i in prange(nnodes):
        for dof in range(degrees_freedom):
            ...
            ud[node_id_i, dof] = ud[node_id_i, dof] + (udd[node_id_i, dof] * dt)
            u[node_id_i, dof] = u[node_id_i, dof] + (ud[node_id_i, dof] * dt)
